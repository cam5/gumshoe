...
2. Fetch the page with cURL to see its raw HTML, for anything crow-nester's default filter left out, or when you specifically need the pre-JS version (see slowcure below). Never substitute a plausible-sounding equivalent for text you haven't actually confirmed in crow-nester's or curl's output: a guess that matches the site's apparent theme (e.g. inventing a restaurant-styled nav for a restaurant site) is still fabrication if it isn't the real content. This applies to icon SVGs and decorative CSS effects too, not just copy — pull the real `<svg>` markup or the real computed filter/hover value from source rather than approximating a plausible-looking stand-in; a hand-drawn icon that looks close is still fabrication if it isn't the actual glyph.
...
The same discipline applies to layout dimensions, not just color: before choosing a Tailwind max-width/container value, or a fixed height for a hero, banner, or header section, grep the fetched CSS for the real rule (a `.wrapper`/container class's actual `max-width`, a section's actual computed height) rather than estimating one from the screenshot. A guessed container width cascades into the horizontal position of everything built against it — nav items, grids, images — not just the one section you were looking at when you guessed it.
...
Before declaring the clone finished, re-run the full-page odiff one final time, after any last-minute fixes — the last comparison you ran must not predate your last edit. If the percentage went up since the previous round, investigate why before finishing rather than reporting the better number from an earlier round: a late fix that looks right can still be a net regression on the page as a whole.
