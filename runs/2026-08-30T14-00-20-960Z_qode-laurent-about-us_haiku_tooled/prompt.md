...
1. Run crow-nester on the URL first, with `--sort-by area` to rank sections by rendered size. Its outline is your map: what sections exist, how big they render, and — for everything it reports — their real text. Note the selectors of anything you'll need a closer look at later (a candidate windtailor target, a suspected JS-driven section, a fragment worth red-twine's judgment).
   Before doing anything else, state a short, explicit priority list from that outline: which two or three sections dominate the rendered page by size and/or text, and therefore deserve the most fidelity effort (a careful windtailor pass, a scoped screenshot-diff at the end), versus which are small or decorative and can be handled quickly by eye. Do this out loud, in your own reasoning — not just implicitly through the order you happen to work in. A miss in whatever crow-nester reports as the largest section (a full-bleed hero, a long-form content block) is the most visible thing wrong with the finished clone; a miss in a small footer icon row is not, and shouldn't get equal attention by default.
...
For this test run: the page to clone is a real, live URL. There is no interactive display, but a
...
absolute path: /var/folders/gb/m6478s0n5mx9wqlz8t_c03800000gn/T/gumshoe-agent-run-S7FpNm/clone.html — do not guess at "the current directory" or
...
/var/folders/gb/m6478s0n5mx9wqlz8t_c03800000gn/T/gumshoe-agent-run-S7FpNm — not /tmp or anywhere else, and use a distinct filename per round
